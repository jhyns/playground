...
export class CanvasApp extends CanvasBase {
  constructor(data = {}) {
    ...
    this.groundFactor = 1 - data.groundFactor || 0.7;
  ...
  drawGround() {
    ...
    this.ctx.fillRect(
      ...
      this.skyHeight,
      this.canvasWidth,
      this.canvasHeight - this.skyHeight
    );
  }
  ...
  drawStar() {
    ...
    this.star.forEach((star, index) => {
      ...
      // this.ctx.closePath()
      if (
        star.x < -star.maxTrailLength ||
        star.y > this.skyHeight + star.maxTrailLength
      ) {
